...
- 提交标题必须说明实际改动，让 GitHub 提交列表不打开详情也能看懂主要变化。
- 复杂改动使用提交正文列出关键行为、兼容处理和验证范围。
- 版本号由 Tag 和项目元数据表达；不要创建仅写 `Release vX.Y.Z`、没有实际改动信息的提交。
- 发布前尽量将版本号、变更日志和对应功能放入同一个有意义的提交，再为该提交创建版本 Tag。
