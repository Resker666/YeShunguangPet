...
- **截图与贴图**：右键角色 → 截图会直接进入区域截图；控制中心或托盘可选择区域、当前屏幕或全部屏幕。在原处调整选区和标注，完成后复制、保存或贴到桌面；托盘“贴图”可管理已有贴图。
...
AI 配置、角色人设与聊天使用说明见 [AI_INTEGRATION.md](docs/AI_INTEGRATION.md)。在控制中心左侧配置「AI 设置」后，右键角色选择「聊天」，即可编辑人设并开始对话。当前默认完全离线，不会自动联网。
