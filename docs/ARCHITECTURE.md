...
`GitHubUpdateChecker` 只在用户手动检查，或明确开启启动检查后，请求本项目的 GitHub Latest Release 元数据。应用默认不联网；检查请求不读取屏幕、角色文件、配置内容或设备标识，也不包含 API Key。

检查器只解析受限大小响应中的版本 Tag，并使用固定仓库地址构造下载页，不执行安装、不下载资产、不替换当前程序。`DesktopSession` 负责可取消的启动检查和托盘提醒；控制中心负责手动状态、显式开关与保存失败回滚。关闭桌面会话会取消仍在进行的请求。
