...
生产源码按现有职责边界组织，仍使用同一个 `YeShunguangPet` 命名空间和程序集：

| 目录 | 职责 |
| --- | --- |
| `AI/` | Provider、密钥存储、角色聊天和 AI 设置 |
| `Capture/` | 屏幕读取、选区、标注、贴图、OCR 与文字助手 |
| `Focus/` | 专注状态机、计时窗口、完成提醒和学习记录 |
| `PetSystem/` | 皮肤包、角色行为、动画、编辑器与角色资源管理 |
| `Desktop/` | 桌面会话、角色窗口、控制中心、诊断、托盘、快捷键和应用设置 |
| `Infrastructure/` | 日志、异常处理、原生 API 和更新检查 |
| `Shared/` | 跨功能复用的主题、动效、外观类型和应用数据路径 |

`App.xaml` 保留在项目根目录作为 WPF 启动入口，`Themes/` 继续存放共享资源字典。`Pets/` 专门保存随发布包分发的角色资源，不放置源码。目录表达代码归属，不建立新的程序集边界，也不改变依赖方向；后续只在具体功能需要时提取接口或缩小职责，避免把文件搬移当作解耦本身。

完整依赖方向、已处理的反向依赖和自动边界见[模块依赖审计](MODULE_DEPENDENCIES.md)。
