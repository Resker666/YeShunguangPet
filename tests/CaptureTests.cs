...
internal static class CaptureTests
{
    ...
    private static void VerifyRegionEditing(Action<bool, string> check, BitmapSource image)
    {
        ...
        var editable = new CaptureDocument(image);
        var editableRectangle = new CaptureMark(CaptureTool.Rectangle, new Point(40, 40), new Point(140, 100), Colors.Red, 4);
        editable.Add(editableRectangle); editable.ReplaceMark(0, editableRectangle.Move(new Vector(20, 15)).Resize(new Rect(60, 55, 160, 90)));
        check(editable.Marks[0].Bounds == new Rect(60, 55, 160, 90), "editable rectangle supports move and eight-handle resize through immutable replacements");
        editable.Undo(); check(editable.Marks[0].Bounds == editableRectangle.Bounds, "object geometry replacement is one undoable history step");
        var arrow = new CaptureMark(CaptureTool.Arrow, new Point(20, 20), new Point(80, 60), Colors.Blue, 3).WithEndpoints(new Point(30, 25), new Point(120, 90));
        check(arrow.Start == new Point(30, 25) && arrow.End == new Point(120, 90), "editable arrow exposes independent start and end points");
        var text = new CaptureMark(CaptureTool.Text, new Point(30, 30), new Point(30, 30), Colors.Black, 2, "old", 20).WithText("updated").Resize(new Rect(40, 45, 200, 60));
        check(text.Text == "updated" && text.Start == new Point(40, 45) && text.FontSize > 20, "editable text supports content replacement, movement and proportional font resizing");
        var pen = new CaptureMark(CaptureTool.Pen, new Point(10, 10), new Point(50, 50), Colors.Green, 3, points: new[] { new Point(10, 10), new Point(30, 20), new Point(50, 50) });
        var resizedPen = pen.Resize(new Rect(20, 30, 80, 40));
        check(resizedPen.Points.Count == 3 && resizedPen.Points[0] == new Point(20, 30) && resizedPen.Points[^1] == new Point(100, 70), "editable pen preserves and scales its complete sampled path");
        editable.RemoveMark(0); check(editable.Marks.Count == 0, "selected annotation can be deleted"); editable.Undo();
        check(editable.Marks.Count == 1, "deleting an annotation is undoable");
